  ...
  for (let s = 1; s <= 150; s++) {
    ...
    for (const inn of m.innings) {
      ...
      inn.ballLog.forEach((b, i) => {
        // an un-split TWO SIXES/TWO FOURS must NEVER reach the log: mid-innings
        // it splits, and on the final ball only the FIRST hit counts ("only")
        if (b.label === "TWO SIXES" || b.label === "TWO FOURS")
          fails.push(`${fmt.key} seed ${s}: un-split compound in log`), fail++;
        ...
        // final-ball compound: one hit only, and it must be the innings' last ball
        if (b.compound === "only") {
          if (b.runs !== 6 && b.runs !== 4) fails.push(`${fmt.key} seed ${s}: 'only' worth ${b.runs}`), fail++;
          if (i !== inn.ballLog.length - 1) fails.push(`${fmt.key} seed ${s}: 'only' not on final ball`), fail++;
        }
      });
    }
  }
